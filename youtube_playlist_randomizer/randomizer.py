...
class Randomizer:
    ...
    def get_playlists(self):
        ...
        self.parse_playlists(response['items'])

    def parse_playlists(self, response):
        for item in response:
            playlist_title = item['snippet']['localized']['title']
            playlist_id = item['id']
            self.own_playlists[playlist_title] = playlist_id

    def choose_playlist_to_randomize(self):
        titles = list(self.own_playlists.keys())
        playlist_title = prompt(
            "Youtube playlist to use ", completer=WordCompleter(titles)
        ).strip()
        playlist_to_create_title = prompt(
            "Youtube playlist to create ", default="{}-randomized".format(playlist_title)
        )
